...
def main():
    ...
    try:
        ...
        if compile_result.returncode != 0:
            print(json.dumps({"output": "", "error": compile_result.stderr.strip(), "exit_code": compile_result.returncode, "resources": {}}))
            return

        result = subprocess.run(
            ["/usr/bin/time", "-f", "USED_TIME=%U;SYS_TIME=%S;ELAPSED=%E;MEM_KB=%M", exe_file],
            stdin=open(input_filename, "r"), capture_output=True, text=True, timeout=5
        )
        ...
        if result.returncode != 0:
            print(json.dumps({"output": program_output, "error": result.stderr.strip(), "exit_code": result.returncode, "resources": resources}))
            return
